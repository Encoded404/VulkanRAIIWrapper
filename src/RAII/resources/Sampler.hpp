...
public:
    ...
    [[nodiscard]] VkSampler GetHandle() const { return sampler_; }
    
    // Implicit conversion to VkSampler
    operator VkSampler() const { return sampler_; }

    // Check if the sampler is valid
    [[nodiscard]] bool IsValid() const { return sampler_ != VK_NULL_HANDLE; }

    // Get sampler properties
    [[nodiscard]] VkFilter GetMagFilter() const { return magFilter_; }
    [[nodiscard]] VkFilter GetMinFilter() const { return minFilter_; }
    [[nodiscard]] VkSamplerMipmapMode GetMipmapMode() const { return mipmapMode_; }
    [[nodiscard]] float GetMaxAnisotropy() const { return maxAnisotropy_; }
    [[nodiscard]] bool IsAnisotropyEnabled() const { return anisotropyEnable_; }

    // Helper constructors for common sampler types
    static Sampler CreateLinear(const Device& device, VkSamplerAddressMode address_mode = VK_SAMPLER_ADDRESS_MODE_REPEAT);
    static Sampler CreateNearest(const Device& device, VkSamplerAddressMode address_mode = VK_SAMPLER_ADDRESS_MODE_REPEAT);
    static Sampler CreateAnisotropic(const Device& device, float max_anisotropy = 16.0f);
    static Sampler CreateShadowMap(const Device& device);
    static Sampler CreateClampToEdge(const Device& device, VkFilter filter = VK_FILTER_LINEAR);
    static Sampler CreateClampToBorder(const Device& device, VkBorderColor border_color = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE);
